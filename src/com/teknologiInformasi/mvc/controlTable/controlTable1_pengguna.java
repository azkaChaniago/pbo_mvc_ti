 ...
 * @author HP
 */
public class controlTable1_pengguna extends AbstractTableModel {
    
    private String[] kolom = {"ID","Nama","Alamat","Telf","Level"};
    public static Vector<model1_Pengguna> feldData=new Vector<model1_Pengguna>(); 
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        switch(columnIndex){             
            case 0: 
                return feldData.get(rowIndex).getId_pengguna();
            case 1: 
                return feldData.get(rowIndex).getNama_pengguna();
            case 2: 
                return feldData.get(rowIndex).getAlamat();             
            case 3: 
                return feldData.get(rowIndex).getTelf();             
            case 4: 
                return feldData.get(rowIndex).getLevel();                                 default: 
                return null;
        }
    }
    ...
    public void insert(model1_Pengguna mdp) {         
        feldData.add(mdp);         
        fireTableRowsInserted(getRowCount(), getRowCount());                 
    }
    
    public void update(int index, model1_Pengguna mdp){         
    ...
    public void lebarKolomTabel(JTable namaTable){         
      ...
      public int search(String id){         
          ...
          while(n<feldData.size() && ketemu==-1){             
                if(id.equals(feldData.get(n).getId_pengguna())) {
                  ketemu=n;
                }             
                n++;         
            }
