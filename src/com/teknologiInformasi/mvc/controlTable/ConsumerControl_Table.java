 ...
 * @author HP
 */
public class ConsumerControl_Table extends AbstractTableModel {

    private String [] column = {"ID", "Nama Konsumen", "Telepon", "Alamat", "Catatan"};
    public static Vector<Consumer_model> field_data = new Vector<Consumer_model>();    
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        switch (columnIndex) {
            case 0:
                return field_data.get(rowIndex).getCon_id();
            case 1:
                return field_data.get(rowIndex).getCon_name();
            case 2:
                return field_data.get(rowIndex).getCon_phone();
            case 3:
                return field_data.get(rowIndex).getCon_address();
            case 4:
                return field_data.get(rowIndex).getCon_notes();
            default :
                return null;
        }
    }
    ...
    public void insert(Consumer_model model) {
        field_data.add(model);
        fireTableRowsInserted(getRowCount(), getRowCount());
    }
    
    public void update(int index, Consumer_model model) {
    ...
    public int search(String id) {
        ...
        while (n < field_data.size() && found == -1) {
            if (id.equals(field_data.get(n).getCon_id())) {
                found = n;
            }
            n++;
        }
